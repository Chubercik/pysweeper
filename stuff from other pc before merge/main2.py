...
from terminal_io import clear, getch, hide_cursor, show_cursor
...
pos = Position(0, 0)  # Default cursor position
...
def draw_image_box(img):
    ...
    box_mid = "│" + " " * (x * 2) + "│"
    box_bot = "╰" + "─" * (x * 2) + "╯"
    draw(TRANSPARENT, Position(x=1 + padding_x, y=offset_y + padding_y), box_top, edge_color)
    for i in range(y):
        draw(
            TRANSPARENT,
            Position(x=1 + padding_x, y=offset_y + padding_y + 1 + i),
            box_mid,
            edge_color,
        )
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=offset_y + padding_y + 1 + y),
        box_bot,
        edge_color,
    )


def draw_image(img, img_pos: Position):
    offset_y = 6
    y, x, _ = img.shape
    ...
    for j in range(y):
        for i in range(x):
            ...
            draw(
                Color(img[j][i]),
                Position(x=i * 2 + 2 + padding_x, y=j + 1 + offset_y + padding_y),
                text,
            )
    sys.stdout.flush()
...
def flood_fill(fill_pos: Position, img, fill_color: Color, original_color):
    img[fill_pos.y][fill_pos.x] = np.array(list(fill_color))
    neighbors = [
        Position(fill_pos.y - 1, fill_pos.x),
        Position(fill_pos.y + 1, fill_pos.x),
        Position(fill_pos.y, fill_pos.x - 1),
        Position(fill_pos.y, fill_pos.x + 1),
    ]
    for i, j in neighbors:
        if (
            ...
            and i < img.shape[0]
            and j < img.shape[1]
            and np.array_equal(img[i][j], list(original_color))
            and not np.array_equal(img[i][j], list(fill_color))
        ):
            img = flood_fill(Position(j, i), np.copy(img), fill_color, original_color)
    return img
...
def color_select(color: Color, offset_y=1):
    ...
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=1 + offset_y + padding_y),
        box_top,
        edge_color,
    )
    for i in range(box_height):
        draw(
            TRANSPARENT,
            Position(x=1 + padding_x, y=1 + offset_y + padding_y + 1 + i),
            box_mid,
            edge_color,
        )
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=1 + offset_y + padding_y + box_height),
        box_bot,
        edge_color,
    )
    ...
    for i in range(len(instructions)):
        draw(
            TRANSPARENT,
            Position(x=1 + padding_x + 1 + i * 12, y=1 + offset_y + padding_y + 1),
            instructions[i],
            secondary_color,
        )
    ...
    for h in range(0, 181, 180 // ticks):
        ...
        text = "●" if round(hsv_color.r / 18) * 18 == h else " "
        draw(
            ncolor_rgb,
            Position(
    ...
    for s in range(0, 251, 250 // ticks):
        ...
        text = "●" if hsv_color.g // (250 / ticks) * 250 // ticks == s else " "
        draw(
            ncolor_rgb,
            Position(
    ...
    for v in range(0, 251, 250 // ticks):
        ...
        ncolor.b = v
        ncolor_rgb = hsv_to_rgb(ncolor)
        text = "●" if hsv_color.b // (250 / ticks) * 250 / ticks == v else " "
        draw(
            ncolor_rgb,
            Position(
    ...
    draw(
        ...
        Position(x=37 + padding_x + 1, y=2 + offset_y + padding_y + 1),
        " " * 11,
    )
...
def draw_interface(filename: str, img) -> None:
    ...
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=1 + padding_y),
    ...
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=8 + padding_y + img.shape[0]),
        "[wasd] move │ [e] draw    │ [f] fill",
        secondary_color,
    )
    draw(
        TRANSPARENT,
        Position(x=1 + padding_x, y=9 + padding_y + img.shape[0]),
...
def main(filepath, resolution):
    ...
    while True:
        ...
        if m == "t":
            ...
            show_cursor()
            clear()
            draw(TRANSPARENT, Position(1, 1), "APPLY FILTER", highlight_color)
            print()
            print("[esc]: Return\n")
            filters = [
            ...
            for i in filters:
                print(f"[{i[0]}]: {i[1]}")
            option = " "
            while option not in [i[0] for i in filters] and option != "\x1b":
                option = getch().upper()
            clear()
            if option != "\x1b":
                history.append(np.copy(img))
        ...
        if m == "\x1b":  # esc
            ...
            show_cursor()
            clear()
            draw(TRANSPARENT, Position(x=1, y=1), "QUIT", highlight_color)
            print()
